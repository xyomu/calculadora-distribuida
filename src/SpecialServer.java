...
public class SpecialServer extends Thread {
	...
	private final static int BASIC_SERVER_PORT = 10002;
	...
	public static void main(String[] args) {
		...
		System.out.println("Expert Server rodando na porta = " + ss.getLocalPort());
		...
		while (true) {
			try {
				...
				Thread thread = new SpecialServer(conexao);
				thread.start();
			} catch (IOException e) {
				e.printStackTrace();
			}
	...
	public double processarResultado(Operacao op) {
		...
		case "^" -> Math.pow(op.getValor1(), op.getValor2());
		case "%" -> (op.getValor1() * op.getValor2()) / 100;
		case "@" -> Math.sqrt(op.getValor1());
		default -> Math.sqrt(op.getValor1());
		};
